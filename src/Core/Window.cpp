...
Window::Window(Instance& instance, const std::string& title, const vk::Extent2D& extent) : mInstance(instance), mTitle(title), mSurface(nullptr), mClientExtent(extent) {
...
std::tuple<vk::raii::PhysicalDevice, uint32_t> Window::FindSupportedDevice() const {
	...
	return std::tuple<vk::raii::PhysicalDevice, uint32_t>( vk::raii::PhysicalDevice(nullptr), uint32_t(-1) );
}
std::vector<uint32_t> Window::FindSupportedQueueFamilies(const vk::raii::PhysicalDevice& physicalDevice) const {
...
void Window::OnInspectorGui() {
	vk::Extent2D e = GetExtent();
	bool changed = false;
